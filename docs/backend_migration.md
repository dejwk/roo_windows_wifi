...
caller-owned provisioning key (default 1). Configure the same key in
`ControllerOptions.startup_profile` to reconnect to that profile after restart.
Applications requiring multiple saved configurations provide their own key map.
...
configurator. The example reserves key 1 for both provisioning and startup.
